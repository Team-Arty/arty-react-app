...
class CNN_Encoder(tf.keras.Model):
    def __init__(self, embedding_dim = 256):
        ...
        w1, w2 = [np.load("api/art_encoder_layer_weights/layer_%s_%s_weights_%s.npy" %(0, "dense", j)) \
                                      for j in range(2)]
...
class BahdanauAttention(tf.keras.Model):
    def __init__(self, units = 512):
        ...
        w1, w2, w3, w4, w5, w6 = [np.load("api/art_decoder_layer_weights/layer_%s_%s_weights_%s.npy" %(4, "bahdanau_attention", j)) \
                                  for j in range(6)]
...
class RNN_Decoder(tf.keras.Model):
    def __init__(self, embedding_dim= 256, units= 512, vocab_size= 10000 + 1):
        ...
        C = tf.keras.initializers.Constant
        w_emb = np.load("api/art_decoder_layer_weights/layer_%s_%s_weights_%s.npy" %(0, "embedding", 0))
        w_gru_1, w_gru_2, w_gru_3 = [np.load("api/art_decoder_layer_weights/layer_%s_%s_weights_%s.npy" %(1, "gru", j)) for j in range(3)]
        w1, w2 = [np.load("api/art_decoder_layer_weights/layer_%s_%s_weights_%s.npy" %(2, "dense_1", j)) for j in range(2)]
        w3, w4 = [np.load("api/art_decoder_layer_weights/layer_%s_%s_weights_%s.npy" %(3, "dense_2", j)) for j in range(2)]
...
def predict_ART(image):
    ...
    with open('api/artemis_tokenizer.json') as f:
        data = json.load(f)
